...
void handleNewMessages(int numNewMessages)
{
	for (int i = 0; i < numNewMessages; i++)
	{
		...
		if (bot.messages[i].longitude != 0 || bot.messages[i].latitude != 0)
		{
			...
			DebugSerial.println(String(bot.messages[i].latitude, 6));
			DebugSerial.print("Long: ");
			DebugSerial.println(String(bot.messages[i].longitude, 6));

            String link = "http://maps.google.com/maps?q=loc:";
			link += String(bot.messages[i].latitude, 6) + ",";
			link += String(bot.messages[i].longitude, 6) + "\n";
			bot.sendMessage(chat_id, link, "Markdown");
		}
		else if (text == "/start")
		{
